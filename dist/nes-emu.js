var NESEmulator = (function (exports) {
    ...
    class PPU {
        constructor(bus) {
            ...
            this.spriteScanline = [];
            this.nextSpriteScanline = []; // For sprites being evaluated for next scanline
    ...
    reset() {
        ...
        clock() {
            ...
            // At the beginning of each visible scanline, swap sprite arrays
            if (isVisibleScanline && this.cycle === 0) {
                // Swap the sprite arrays - sprites evaluated last scanline are now current
                this.spriteScanline = this.nextSpriteScanline;
                this.nextSpriteScanline = [];
            }
            
            // For scanline 0, we need special handling since there was no evaluation for scanline -1
            // Initialize with sprites that should be displayed on scanline 0
            if (this.scanline === 0 && this.cycle === 0 && this.spriteScanline.length === 0) {
                // Manually check which sprites should be on scanline 0
                const spriteHeight = (this.control & 0x20) ? 16 : 8;
                for (let i = 0; i < 64; i++) {
                    const oamEntryY = this.oam[i * 4];
                    if (0 >= oamEntryY && 0 < oamEntryY + spriteHeight) {
                        if (this.spriteScanline.length < 8) {
                            const sprite = {
                                y: oamEntryY,
                                id: this.oam[i * 4 + 1],
                                attr: this.oam[i * 4 + 2],
        ...
        loadBackgroundShifters() {
            this.bgShifterPatternLo = (this.bgShifterPatternLo & 0xFF) | this.bgNextTileLsb;
            this.bgShifterPatternHi = (this.bgShifterPatternHi & 0xFF) | this.bgNextTileMsb;
            this.bgShifterAttribLo = (this.bgShifterAttribLo & 0xFF) | ((this.bgNextTileAttr & 0x01) ? 0xFF : 0x00);
            this.bgShifterAttribHi = (this.bgShifterAttribHi & 0xFF) | ((this.bgNextTileAttr & 0x01) ? 0xFF : 0x00);
        }
        ...
        fetchAttributeTableByte() {
            ...
            // Select the relevant 2-bit attribute based on coarse X/Y
        ...
        incrementScrollY() {
            if ((this.addr & 0x7000) !== 0x7000) {
                this.addr += 0x1000;
            } else {
                this.addr &= ~0x7000;
                let y = (this.addr & 0x03E0) >> 5;
                if (y === 29) {
                    y = 0;
                    this.addr ^= 0x0800;
                } else {
                ...
                this.addr = (this.addr & ~0x03E0) | (y << 5);
            }
        }
        
        transferX() {
            this.addr = (this.addr & ~0x041F) | (this.tempAddr & 0x041F);
        }
        
        transferY() {
            this.addr = (this.addr & ~0x7BE0) | (this.tempAddr & 0x7BE0);
        }
        ...
        pixelRendering() {
            ...
            if (this.mask & 0x10) { // Sprite rendering enabled
                ...
                for (let i = 0; i < this.spriteScanline.length; i++) {
                    ...
                    if (spriteData.pixel !== 0) { // Non-transparent sprite pixel
                        ...
                        // Check for sprite 0 hit - hardware sets flag when sprite 0 pixel overlaps background pixel
            ...
            // Sprite 0 hit detection - simplified to hardware behavior
            if (this.spriteZeroPossible && bgOpaque && spritePixel !== 0) {
                if (!(this.status & 0x40)) { // Only set once per frame
        ...
        evaluateSpritesStep() {
            ...
            if (cycleOffset === 0) {
                ...
                if (this.spriteEvaluationIndex < 64) {
                    ...
                    // Check if sprite is on NEXT scanline (evaluation for next line)
                    const nextScanline = this.scanline + 1;
                    if (nextScanline >= oamEntryY && nextScanline < oamEntryY + spriteHeight) {
                        if (this.foundSprites < 8) {
                            ...
                            // Add to next sprite scanline for rendering
                            const sprite = {
                                y: oamEntryY,
                            ...
                            this.nextSpriteScanline.push(sprite);
            ...
            // Set overflow flag when 9th sprite is found during evaluation
            if (this.spriteOverflowIndex >= 0 && this.foundSprites === 8) {
        ...
        getColorFromPalette(palette, index) {
            // Palette 0 is background palette, palettes 1-7 are sprite palettes
            // Each palette has 4 colors: 0=transparent, 1-3=actual colors
            if (palette === 0) {
                // Background palette - use palette RAM
                return this.palette[index & 0x0F];
            } else {
                // Sprite palette - use palette RAM offset by sprite palette index
                const paletteIndex = ((palette - 1) << 2) + (index & 0x03);
                return this.palette[paletteIndex & 0x1F];
            }
        }
